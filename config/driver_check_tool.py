...
def get_chrome_driver_list(current_chrome_version):
    ...
    log.info('可下载的driver 版本号：', driver_version_nums)
    ...
    try:
        ...
        if res:
            ...
            if os.system('file {}/chromedriver'.format(download_path)) == 0:
                log.info('chromedriver 类型修改成功!')
                # 复制文件到 usr/local/bin
                copy_file(download_path + '/chromedriver', chrome_driver_path)

                log.info('chrome driver 版本更换成功！！！')
            else:
                log.error('chromedriver 类型修改失败，无法进行文件复制！')
        else:
            log.error('报错信息中未匹配到 浏览器版本！')
    finally:
        if driver:
            driver.quit()
